...
m_GDP_M = ols('y ~ RGDP_M',housing_df).fit()
print(m_GDP_M.summary())


m_rcpi = ols('y ~ H_RATIO_3A + HSN1F_3A + PERMIT_3A + STOCK_MKT_3A + BAA_YEILD_10Y_2A + US10Y_3A + RPCE_A + UEMP_3A + RGDP_M',housing_df).fit()
...
Extract the column names of the jobs DataFrame and place the results in a list called h_m_df_names.
...
housing_subset_1 = housing_df['1979-01-01':'1982-01-01']
...
predictors = np.array(h_m_s_df.drop(['ASPUS_M'],1))
target = np.array(h_m_s_df['ASPUS_M'])
...
'''There we go…a forecast for Housing Price 60 months into the future (you have to look closely at the very far right-hand side for the forecast). It looks like the next sixths months will see sales between 350K and 555K.'''
